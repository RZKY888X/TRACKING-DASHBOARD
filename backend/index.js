  ...
  /* ======================================================
    ASSIGNMENT APIs
    - provide users list for assignment (includes jobRole)
  ====================================================== */
...
// backend/index.js
// Add these Assignment APIs before the server start section

/* ======================================================
   ASSIGNMENT APIs (NEW & CORRECTED)
====================================================== */
...
// POST: Create new assignment (CORRECTED VERSION)
app.post("/api/assignments", authMiddleware, async (req, res) => {
  try {
    ...
    if (!user && email) {
      // Create user if doesn't exist but has email
      ...
      return res.status(404).json({ 
        error: "User tidak ditemukan dan email tidak diberikan" 
      });
    }
    ...
    // Update vehicle driver if empty
    ...
    await writeActivityLog({
      ...
      description: `Created assignment for ${fullName} with vehicle ${vehicle}`,
      req,
      metadata: {
        userName: fullName,
        vehicle: vehicle,
