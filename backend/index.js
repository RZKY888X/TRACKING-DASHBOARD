// backend/index.js - COMPLETE VERSION WITH AUTH
...
/* ======================================================
   MQTT CLIENT (IoT → DB)
====================================================== */
// OFF: Comment MQTT if broker not running
...
app.get("/api/activity/stats", authMiddleware, roleMiddleware(["ADMIN", "SUPERADMIN"]), async (req, res) => {
  try {
    ...
    const activityByUserWithDetails = activityByUser.map((a) => ({ 
    ...
    res.json({ activityByAction, activityByUser: activityByUserWithDetails, timeline });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.get("/api/stats", authMiddleware, async (req, res) => {
  try {
    ...
    const avgSpeed = latest.length > 0 
      ? (latest.reduce((acc, v) => acc + v.speed, 0) / latest.length).toFixed(1) 
      : 0;
