...
export default function FilterSection({
  ...
  const fetchCascadeOptions = async () => {
    try {
      ...
      const res = await fetch(`${API_URL}/api/filter/cascade-options?${params}`, {
      ...
      if (res.ok) {
        ...
        if (data.success) {
          ...
          const formattedDestinations = (data.destinations || []).map((d: any) => ({
            id: d.id,
            name: d.name,
            city: d.city || "",
  ...
  const getFilteredRoutes = () => {
    ...
    if (searchRoute) {
      filtered = filtered.filter(origin => 
        origin.name.toLowerCase().includes(searchRoute.toLowerCase()) ||
        origin.displayName?.toLowerCase().includes(searchRoute.toLowerCase()) ||
        (origin.city && origin.city.toLowerCase().includes(searchRoute.toLowerCase()))
      );
    }
  ...
  const getFilteredDepartures = () => {
    ...
    if (searchDeparture) {
      filtered = filtered.filter(destination => 
        destination.name.toLowerCase().includes(searchDeparture.toLowerCase()) ||
        destination.displayName?.toLowerCase().includes(searchDeparture.toLowerCase()) ||
        (destination.city && destination.city.toLowerCase().includes(searchDeparture.toLowerCase()))
      );
    }
  ...
  const getDateLabel = () => {
    if (!filters.dateType || filters.dateType === "select") return "Select Date";
    ...
    if (filters.dateType === "weekly" && filters.dateValue) {
      ...
      if (parts.length === 2) {
        ...
        return `Week ${weekNum}, ${new Date(parseInt(year), parseInt(month) - 1).toLocaleDateString('id-ID', {
          month: 'long',
          year: 'numeric'
        })}`;
      }
  ...
  return (
    <div className="bg-[#0D1117] border border-[#1F2A37] rounded-lg p-6 relative z-10" ref={dropdownRef}>
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-cyan-400 mb-2">Filter Data</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="relative">
          <label className="block text-sm text-gray-400 mb-2">Select Date Range</label>
          ...
          >
            ...
            <svg
              ...
              className={`w-5 h-5 text-cyan-400 transition-transform ${openDate ? 'rotate-180' : ''}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
                        ...
                        {weeks.map((week) => {
                          ...
                          return (
                            <button
                              key={week}
                              onClick={() => {
                                handleChange("dateValue", `${selectedMonth} Week ${weekNum}`);
                                setMode("select");
                                setOpenDate(false);
                              }}
                              className={`w-full text-left px-3 py-2 text-sm rounded transition ${
                                filters.dateValue === `${selectedMonth} Week ${weekNum}`
                                  ? 'bg-[#1F2937] text-cyan-400 border border-cyan-500/30'
        ...
        <div className="relative">
          ...
          <div className="relative">
            <button
              ...
              disabled={loading || !filters.dateType || filters.dateType === "select" || filterOptions.drivers.length === 0}
              title={
                !filters.dateType || filters.dateType === "select" 
                  ? "Select date first" 
                  : filterOptions.drivers.length === 0 
            ...
            >
              ...
              {filterOptions.drivers.length > 0 && !loading && filters.dateType && filters.dateType !== "select" && (
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`w-5 h-5 transition-transform ${openDriver ? 'rotate-180' : ''} text-cyan-400`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
                  ...
                  filteredDrivers.map((driver) => (
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        <div className={`w-2 h-2 rounded-full ${filters.driver === driver.name ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
        ...
        <div className="relative">
          ...
          <div className="relative">
            <button
              onClick={() => {
                if (filterOptions.origins.length === 0 || !filters.driver) return;
              ...
              disabled={loading || !filters.driver || filterOptions.origins.length === 0}
              title={
                !filters.driver 
                  ? "Select driver first" 
                  : filterOptions.origins.length === 0 
            ...
            >
              ...
              {filterOptions.origins.length > 0 && !loading && filters.driver && (
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`w-5 h-5 transition-transform ${openRoute ? 'rotate-180' : ''} text-cyan-400`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
              ...
              <div className="max-h-60 overflow-y-auto">
                {filteredRoutes.length > 0 ? (
                  filteredRoutes.map((origin) => (
                    <button
                      key={origin.id}
                      onClick={() => handleChange("route", formatDisplayName(origin))}
                      className={`w-full text-left px-3 py-2 text-sm transition flex items-center justify-between ${
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        <div className={`w-2 h-2 rounded-full ${filters.route === formatDisplayName(origin) ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
                        <span className="truncate">{formatDisplayName(origin)}</span>
                      </div>
                      {filters.route === formatDisplayName(origin) && (
                        <svg className="w-4 h-4 text-cyan-400" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
        ...
        {/* ================= DEPARTURE ================= */}
        <div className="relative">
          <label className="block text-sm text-gray-400 mb-2">Departure (Destination)</label>
          <div className="relative">
            <button
              onClick={() => {
                if (filterOptions.destinations.length === 0 || !filters.driver || !filters.route) return;
              ...
              disabled={loading || !filters.driver || !filters.route || filterOptions.destinations.length === 0}
              title={
                !filters.driver 
                  ? "Select driver first" 
                  : !filters.route 
            ...
            >
              ...
              {filterOptions.destinations.length > 0 && !loading && filters.driver && filters.route && (
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`w-5 h-5 transition-transform ${openDeparture ? 'rotate-180' : ''} text-cyan-400`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
          ...
          {openDeparture && filterOptions.destinations.length > 0 && filters.driver && filters.route && (
            <div className="absolute mt-2 w-full bg-[#0D1117] border border-[#1F2A37] rounded-lg z-50 shadow-xl">
              <div className="p-2 border-b border-[#1F2A37]">
                <div className="relative">
                  <svg className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="max-h-60 overflow-y-auto">
                {filteredDepartures.length > 0 ? (
                  filteredDepartures.map((destination) => (
                    <button
                      key={destination.id}
                      onClick={() => handleChange("departureRoute", formatDisplayName(destination))}
                      className={`w-full text-left px-3 py-2 text-sm transition flex items-center justify-between ${
                        filters.departureRoute === formatDisplayName(destination)
                          ? 'bg-[#1F2937] text-cyan-400'
                          : 'hover:bg-[#1F2937] text-gray-200'
                      }`}
                    >
                      <div className="flex items-center space-x-2">
                        <div className={`w-2 h-2 rounded-full ${filters.departureRoute === formatDisplayName(destination) ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
                        <span className="truncate">{formatDisplayName(destination)}</span>
                      </div>
                      {filters.departureRoute === formatDisplayName(destination) && (
                        <svg className="w-4 h-4 text-cyan-400" fill="currentColor" viewBox="0 0 20 20">
                          <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
      ...
      <div className="mt-4 pt-4 border-t border-[#1F2A37]">
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center space-x-4">
            <div className={`flex items-center space-x-2 ${filters.dateType && filters.dateType !== "select" ? 'text-cyan-400' : 'text-gray-500'}`}>
              <div className={`w-2 h-2 rounded-full ${filters.dateType && filters.dateType !== "select" ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
              <span>Date: {filters.dateType && filters.dateType !== "select" ? 'Selected' : 'Not selected'}</span>
            </div>
            <div className={`flex items-center space-x-2 ${filters.driver ? 'text-cyan-400' : 'text-gray-500'}`}>
              <div className={`w-2 h-2 rounded-full ${filters.driver ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
              <span>Driver: {filters.driver ? 'Selected' : 'Not selected'}</span>
            </div>
            <div className={`flex items-center space-x-2 ${filters.route ? 'text-cyan-400' : 'text-gray-500'}`}>
              <div className={`w-2 h-2 rounded-full ${filters.route ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
              <span>Origin: {filters.route ? 'Selected' : 'Not selected'}</span>
            </div>
            <div className={`flex items-center space-x-2 ${filters.departureRoute ? 'text-cyan-400' : 'text-gray-500'}`}>
              <div className={`w-2 h-2 rounded-full ${filters.departureRoute ? 'bg-cyan-400' : 'bg-gray-600'}`}></div>
              <span>Departure: {filters.departureRoute ? 'Selected' : 'Not selected'}</span>
            </div>
          </div>
          {loading && (
